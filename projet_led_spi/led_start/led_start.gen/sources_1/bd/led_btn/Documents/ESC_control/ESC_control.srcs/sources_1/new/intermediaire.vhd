...
entity pwm_intermediate is
    generic (
        MAX_PWM_INITIAL : unsigned(9 downto 0) := "0001100100" -- Valeur max par défaut (280)
    );
    Port (
...
architecture Behavioral of pwm_intermediate is
    ...
    signal max_pwm : unsigned(9 downto 0) := MAX_PWM_INITIAL;     -- Valeur max initialisée avec le generic
...
begin
    ...
    begin
        if reset = '0' then
            ...
            init_counter <= 0;
            init_pwm_done <= '0';
            max_pwm <= MAX_PWM_INITIAL; -- Réinitialiser max_pwm avec la valeur initiale
    ...
    -- Processus pour détecter les fronts montants de add_speed et remove_speed
    process(clk, reset)
    begin
        if reset = '0' then
            add_speed_prev <= '0';
            ...
            -- Détection du front montant de add_speed
            ...
            -- Détection du front montant de remove_speed
